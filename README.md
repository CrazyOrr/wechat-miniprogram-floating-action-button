# wechat-miniprogram-floating-action-button

微信小程序组件 floating-action-button
...
```
npm install @crazyorr/wechat-miniprogram-floating-action-button
```

2. 在页面的 json 配置文件中添加 floating-action-button 自定义组件的配置

```
{
  "usingComponents": {
    "floating-action-button": "@crazyorr/wechat-miniprogram-floating-action-button/floating-action-button"
  }
}
```

3. 在页面的 wxml 文件中引用 floating-action-button
...
4. 样式定制，可使用 `ext-class` 参数传入页面中定义的样式类名称
```
<floating-action-button ext-class="fab"></floating-action-button>
```
```
.fab {
    background-color: red !important; // must be more specific than default css class
}
```
